...
class EcObsDataset(Dataset):
    ...
    def __init__(self, start_year: int = 2015, end_year: int = 2020, x_slice_indices: tuple = (0, None),
        ...
        self.ds_sm_obs = zarr.open(root_sm)
        self.ds_temp_obs = zarr.open(root_temp)
        if use_time_var_lai:
            self.lail = zarr.open(root_lail)
            self.laih = zarr.open(root_laih)

        # create time index to select appropriate data range
        date_times_ecland = pd.to_datetime(cftime.num2pydate(self.ds_ecland["time"], self.ds_ecland["time"].attrs["units"]))
        # create time index to select appropriate data range
        self.start_index = min(np.argwhere(date_times_ecland.year == int(start_year)))[0]
        self.end_index = max(np.argwhere(date_times_ecland.year == int(end_year)))[0]
        ...
        self.times = np.array(date_times_ecland[self.start_index: self.end_index + 1], dtype=np.datetime64)
        ...
        # for observational/time-variable datasets
        self.sm_index = list(self.ds_ecland["variable"]).index("swvl1")
        self.temp_index = list(self.ds_ecland["variable"]).index("skt")
        # if use_time_var_lai:
        #     self.lail_index = list(self.ds_ecland["variable"]).index("lai_lv")
        #     self.laih_index = list(self.ds_ecland["variable"]).index("lai_hv")
        ...
        if is_norm:
            ...
            # get statistic to normalize the ioncrement to the soil moisture observation
    ...
    def __getitem__(self, idx):
       ...
       """
        ...
        # Replace LAI fields with time variable data
        ...
        # print(data_sm.shape, data_temp.shape)
