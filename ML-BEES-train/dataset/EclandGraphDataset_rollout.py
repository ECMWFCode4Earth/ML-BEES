...
class EcDataset(Dataset):
    ...
    def __init__(self, start_year: int = 2015, end_year: int = 2020, x_slice_indices: tuple = (0, None),
        ...
        self.data_static = np.repeat(self.data_static, self.roll_out, axis=0)
    ...
    def _knn_graph(lat: np.array, lon: np.array, k: int) -> np.array:
        ...
        edge_index[1, :] = np.repeat(np.arange(0, len(lat)), k)
        ...
        for i in range(len(lat)):
            ...
            edge_index[0, (i*k):(i*k)+k] = ind_knn
    ...
    def __getitem__(self, idx):
       ...
       """
        ...
        return Data(data_dynamic=torch.from_numpy(data_dynamic[:-1]).permute(1, 0, 2),
                    data_prognostic=torch.from_numpy(data_prognostic[:-1]).permute(1, 0, 2),
                    data_prognostic_inc=torch.from_numpy(data_prognostic_inc).permute(1, 0, 2),
                    data_diagnostic=torch.from_numpy(data_diagnostic[:-1]).permute(1, 0, 2),
                    data_static=torch.from_numpy(data_static).permute(1, 0, 2),
                    data_time=torch.from_numpy(data_time).unsqueeze(1).repeat(1, self.x_size, 1).permute(1, 0, 2),
...
if __name__ == "__main__":
    ...
    print('edge index shape:', dataset.__getitem__(0).edge_index.shape)
    print()
